...
# Load data 
eval_data = '/work2/jcintineo/torcnn/tests/2011-24/test01/eval2018/eval_data.csv'
...
print(f"ALL New Max CSI: {csi[ix]:.4f} at Threshold: {thresholds[ix]:.4f}")
...
csi = 1 / (1/precision + 1/recall - 1 + 1e-10)
ix = np.argmax(csi)
print(f"Pretor vs. nontor New Max CSI: {csi[ix]:.4f} at Threshold: {thresholds[ix]:.4f}\n")
...
csi = 1 / (1/precision + 1/recall - 1 + 1e-10)
ix = np.argmax(csi)
print(f"Pretor (1-10 min) vs. nontor New Max CSI: {csi[ix]:.4f} at Threshold: {thresholds[ix]:.4f}\n")
...
csi = 1 / (1/precision + 1/recall - 1 + 1e-10)
ix = np.argmax(csi)
print(f"Pretor (11-20 min) vs. nontor New Max CSI: {csi[ix]:.4f} at Threshold: {thresholds[ix]:.4f}\n")
...
csi = 1 / (1/precision + 1/recall - 1 + 1e-10)
ix = np.argmax(csi)
print(f"Pretor (21-30 min) vs. nontor New Max CSI: {csi[ix]:.4f} at Threshold: {thresholds[ix]:.4f}")

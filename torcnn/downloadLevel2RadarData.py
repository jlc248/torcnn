...
def main(outputDir = "temp", dateFormat = "%Y%m%d-%H%M", startDates = [], endDates = [],
   ...
   objectsInBucket = np.array([noaas3.list_objects_v2(Bucket = "unidata-nexrad-level2",
   ...
   for file in tqdm(filesToDownload, file = sys.__stdout__):
      if not path.exists(outputDir + '/' + file[file.rfind('/') + 5:file.rfind('/') + 13] +
         ...
         system("chmod 775 {}/{}{}/raw".format(outputDir,file[file.rfind('/') + 5:file.rfind('/') + 13],
                                              file[file.rfind('/'):file.rfind('/') + 5]))
      
      noaas3.download_file("unidata-nexrad-level2", file,
                           outputDir + '/' + file[file.rfind('/') + 5:file.rfind('/') + 13] +
                           file[file.rfind('/'):file.rfind('/') + 5] + "/raw" + file[file.rfind('/'):])
      system("chmod 775 {}/{}{}/raw{}".format(outputDir,file[file.rfind('/') + 5:file.rfind('/') + 13],
                                              file[file.rfind('/'):file.rfind('/') + 5], file[file.rfind('/'):]))
...
def useCSV(file, radarName, radarSep, domainName, domainSep, timeStampName, startTimeName, endTimeName,
   ...
   if startTimeName in df and endTimeName in df:
      epochTime = np.array([{"start" : timegm(time.strptime(s, dateFormat)) - timeThreshold,
                             "end" : timegm(time.strptime(e, dateFormat)) + timeThreshold,
...
def pullNSE(directory, files, copyST, copyNRE):
   ...
   for p in tqdm(paths, file = sys.__stdout__):
      ...
      if copyST:
         ...
         try:
            print("\nrsync -auq wdssii@epvc.hwt.nssl:/hwt/NSE_processing/NSE_organized/{} "
                  "{}/{}/NSE/SoundingTable/{}/".format(p, directory, yyyymmdd, radar), flush = True)
            system("rsync -auq wdssii@epvc.hwt.nssl:/hwt/NSE_processing/NSE_organized/{} {}/{}/NSE/SoundingTable/{}/ "
         ...
         if not glob("{}/{}".format(directory, p)):
            print("Could not find {} at epvc.".format(p))

         # Code for when data was not all in one place. 
         #if not glob("{}/{}".format(directory, p)):
         #   print("{}/{}".format(directory, p))
         #   try:
         #      print("\nrsync -auq wdssii@epvc.hwt.nssl:/data6/NSE_Rebuild/NSE_organized/{} "
         #            "{}/{}/NSE/SoundingTable/{}/".format(p, directory, yyyymmdd, radar), flush = True)
         #      system("rsync -auq wdssii@epvc.hwt.nssl:/data6/NSE_Rebuild/NSE_organized/{} "
      ...
      if copyNRE:
         ...
         try:
            ...
            system("rsync -auq wdssii@epvc.hwt.nssl:/hwt/NSE_processing/NSE_organized/{} {}/{}/NSE/NearRadarEnvironmentTable/{}/ "
                   "> /dev/null 2>&1".format(p, directory, yyyymmdd, radar))
...
if __name__ == "__main__":
   ...
   parser.add_argument("--rt", metavar = "latName", type = str, nargs = '?', default = '',
                       help = "Specify latitude column name for radar file if "
   ...
   parser.add_argument("--rn", metavar = "lonName", type = str, nargs = '?', default = '',
                       help = "Specify longitude column name for radar file "
   ...
   parser.add_argument("--rr", metavar = "radarCol", type = str, nargs = '?', default = '',
                       help = "Specify radar column name for radar file if "
